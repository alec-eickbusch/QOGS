...
class GateSynthesizer:
    ...
    def batch_state_transfer_fidelities(self, opt_params: Dict[str, tf.Variable]):
        ...
        bs = self.gateset.batch_construct_block_operators(opt_params) # (N_blocks, N_batch, N, N)
        psis = tf.stack([self.initial_states] * self.parameters["N_multistart"])
        # print(psis.shape)
        ein_str = ""
        shapes = [(self.parameters["N_multistart"], bs.shape[-2], bs.shape[-1])] * N_blocks
        for k in range(N_blocks):
            # N_blocks + 2 is m/batch
            # N_blocks + 3 is s/state
            # N_blocks + 4 is i/vector index
            # N_blocks + 5 is k/extra index
            ein_str += "{}{}{},".format(oe.get_symbol(N_blocks + 2), oe.get_symbol(k), oe.get_symbol(k + 1)) # this creates the 
        ein_str += "{}{}{}{}->".format(oe.get_symbol(N_blocks + 2), oe.get_symbol(N_blocks + 3), oe.get_symbol(N_blocks), oe.get_symbol(N_blocks + 5))
        ein_str += "{}{}{}{}".format(oe.get_symbol(N_blocks + 2), oe.get_symbol(N_blocks + 3), oe.get_symbol(0), oe.get_symbol(N_blocks + 5))
        shapes.append(tuple(psis.get_shape().as_list()))
        matrix_contract = oe.contract_expression(ein_str, *shapes)

        # collect the N_blocks axis into a list
        to_contract = [bs[k, :, :, :] for k in np.arange(N_blocks)]
        to_contract.append(psis)

        psis_final = matrix_contract(*to_contract, backend='tensorflow')

        overlaps = tf.einsum("si...,msi...->ms...", self.target_states_dag, psis) # calculate overlaps
    ...
    def batch_state_transfer_fidelities_real_part(self, opt_params: Dict[str, tf.Variable]):
        ...
        ein_str += "{}{}{}{}".format(oe.get_symbol(N_blocks + 2), oe.get_symbol(N_blocks + 3), oe.get_symbol(0), oe.get_symbol(N_blocks + 5))
        shapes.append(tuple(psis.get_shape().as_list()))
        matrix_contract = oe.contract_expression(ein_str, *shapes) # probably don't need to use oe for this, maybe just use it to create the contraction string

        # collect the N_blocks axis into a list
        to_contract = [bs[k, :, :, :] for k in np.arange(N_blocks)] # this may be slow
        to_contract.append(psis)
